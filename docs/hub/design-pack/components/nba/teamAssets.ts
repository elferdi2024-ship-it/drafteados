...
export function getNbaLogoUrl(
  teamIdOrTricode: number | string,
  variant: "primary" | "global" = "primary"
): string {
  let teamId: number;
  if (typeof teamIdOrTricode === "number") {
    teamId = teamIdOrTricode;
  } else {
    const team = getTeamByTricode(teamIdOrTricode);
    if (!team) {
      // fallback vacío — el componente usará onError
      return "";
    }
    teamId = team.teamId;
  }
